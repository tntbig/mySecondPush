...
public class RecommendationFragment extends Fragment {
			...
			mUrl.setOnClickListener(new android.view.View.OnClickListener() {
				...
				public void onClick(View view1) {
					if (mRecommendation == null) {
						...
						Intent intent = new Intent("android.intent.action.VIEW", Uri.parse(mRecommendation.getContent()));
	...
	public void onStart() {
		...
		OffersKit.getInstance().recommendation(mRecommendation_id, true, new OffersListener() {
			...
			public void onDone(Map<String, Object> map) {
				...
				if(offersstatus != null) {
					if(offersstatus.getCode() != 0){
						((MainActivity) getActivity()).alert("recommendation.onDone", offersstatus.getCode() + ":" + offersstatus.getMessage());
					}
				}

				if (map.get("recommendation") != null) {
					mRecommendation = (OffersRecommendation) map.get("recommendation");
					mRecommendation.actionType("open", new OffersListener() {
					...
					mDescription.setText("説明:" + mRecommendation.getDescription());
					mDate.setText((new StringBuilder("利用期間:")).append(mRecommendation.getDeliveryFromAt()).append(" 〜 ").append(mRecommendation.getDeliveryToAt()).toString());
					TextView textview = mUnread;
					String s;
					if (mRecommendation.isUnread()){
						s = "未読";
					...
					System.out.println("getContent:" + mRecommendation.getContent());
						...
						OffersKit.getInstance().authenticationToken(new OffersListener(){
							...
							public void onDone(Map<String, Object> result) {
								if( result.get("token") != null){
									mContent.loadDataWithBaseURL("http://?token=" + result.get("token") , mRecommendation.getContent(), "text/html", "utf-8", null);
								}else{
					...
					//mContent.loadData(mRecommendation.getContent(), "text/html; charset=utf-8", "utf-8");
